...
use i18n_country_translations_data::ALL_LOCALES;
...
impl Registry {
    ...
    /// Get country name using default locale. Code is auto-uppercased.
    pub fn get_name(&self, code: &str) -> Option<&str> {
        let locale = self.default_locale.as_deref()?;
        self.get_name_for_locale(locale, code)
    }

    /// Get country name for a specific locale. Code is auto-uppercased.
    pub fn get_name_for_locale(&self, locale: &str, code: &str) -> Option<&str> {
